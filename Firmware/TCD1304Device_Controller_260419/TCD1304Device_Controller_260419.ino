...
/*
  ...
  Contact:  drmcnelsonlab@gmail.com
     ...
     Copyright 2020-2026 Mitchell C. Nelson, PhD
...
{
  uint8_t *pc = (unsigned char *)&bufferp[SENDSTART];
  uint8_t crc = 0xff;
  size_t i;
  for (i = 0; i < SENDLENGTHBYTES; i++) {
    crc ^= *pc++;
    crc = crctable[crc];
  }
  return crc;
...
inline uint32_t sumData(uint16_t *bufferp) {
  uint16_t *p16 = &bufferp[SENDSTART];
  uint32_t u = 0;
  int n = 0;
  for (n = 0; n < SENDLENGTHWORDS; n++) {
    u += *p16++;
  }
  return u;
}
...
void sendBuffer_Formatted(uint16_t *bp) {
  uint16_t *p16 = &bp[SENDSTART];
  Serial.print("DATA ");
  Serial.println(SENDLENGTHWORDS);
  for (int n = 0; n < SENDLENGTHWORDS; n++) {
...
void sendBuffer_Binary(uint16_t *bp) {
  ...
  Serial.println(SENDLENGTHWORDS);
  Serial.write((byte *) p16, SENDLENGTHBYTES);
  Serial.send_now();
  Serial.println("END DATA");
}
...
void send_header()
{
  ...
  switch(tcd1304device.mode) {
  case PULSE:
    Serial.print(" PULSE");
    break;
  case PULSE_LOOP:
    Serial.print(" PULSE_LOOP");
    break;
  case PIT:    
    Serial.print(" PIT");
    break;
  ...
  if (tcd1304device.mode == PIT) {
    Serial.print("TIMER PERIOD ");
    Serial.println((int)(tcd1304device.pit_interval_secs*1.E6));

    Serial.print("TIMER DELAY ");
    Serial.println((int)(tcd1304device.pit_delay_secs*1.E6));    
  }
...
bool send_frame(TCD1304Device :: Frame_Header *p)
{
  ...
  Serial.printf("FRAMESET COUNTER %d\n",p->frameset_counter);

  Serial.printf("FRAME COUNTER %d\n",p->frame_counter);

  Serial.printf("FRAME ELAPSED %.8f\n",p->frame_elapsed_secs);

  Serial.printf("FRAME EXPOSURE %.8f\n",p->frame_exposure_secs);

#ifdef DEBUG  
  Serial.printf("FRAME_DIFFERENCE %.8f",p->frame_difference_secs);
#endif
  
  if (p->mode==PIT) {
    Serial.printf("TIMER ELAPSED %.8f\n",p->pit_elapsed_secs);
    Serial.printf("TIMER DIFFERENCE %.8f\n",p->pit_difference_secs);
  }
    
  if (p->trigger_mode) {
    Serial.printf("TRIGGER ELAPSED %.8f\n",p->trigger_elapsed_secs);
    Serial.printf("TRIGGER DIFFERENCE %.8f\n",p->trigger_difference_secs);
    Serial.printf("TRIGGER COUNTER %d\n",p->trigger_counter);
  }

  if (p->offsetdone) {
    Serial.printf("RAWOFFSET %d\n",p->rawoffset);
...
void strWaitParameters(char *pc, float *timeout, float*timestep, bool *interruptible, bool *verbose, char **next=0)
{
  float *pval = timeout;
  ...
  while (pc && (*pc)) {
    ...
    else if (strMatch(pc, "not verbose", &pc)) {
      *verbose = false;
    }
    else if (strMatch(pc, "interruptible", &pc)) {
      *interruptible = true;
    }
    else if (strMatch(pc, "not interruptible", &pc)) {
      *interruptible = false;
    }
    else if (strFlt(pc,pval, &pc)) {
      pval = timestep;
    }    
...
bool strTimings(char *s, char **next=0)
{
  float *duration = NULL;
  float *offset = NULL;
  
  Serial.print("strTimings top "); Serial.println(s);
  ...
  while (s && *s) {
    ...
    Serial.print("strTimings next "); Serial.println(s);
    
    if (strMatch(s,"clk",&s)) {
      Serial.print("parsed clk next"); Serial.println(s);
      if (!strFlt(s,&tcd1304device.pulse_clk_secs,&s)) {
        return false;
      }
      duration = NULL;
      offset = NULL;
      ...
      Serial.print("parsed period next"); Serial.println(s);
      if (!strFlt(s,&tcd1304device.pulse_loop_period_secs,&s)) {
        return false;
      }
    }
    else if (strMatch(s,"sh",&s)) {
      Serial.print("parsed sh next"); Serial.println(s);
      duration = &tcd1304device.pulse_sh_secs;
      offset = &tcd1304device.pulse_sh_offset_secs;
    }
    else if (strMatch(s,"icg",&s)) {
      Serial.print("parsed icg next"); Serial.println(s);
      duration = &tcd1304device.pulse_icg_secs;
      offset = &tcd1304device.pulse_icg_offset_secs;
    }
    else if (strMatch(s,"cnvst",&s)) {
      Serial.print("parsed cnvst next"); Serial.println(s);
      duration = &tcd1304device.pulse_cnvst_secs;
      offset = &tcd1304device.pulse_cnvst_offset_secs;
    }
    else if (strMatch(s,"offset",&s)) {
      Serial.print("parsed offset next "); Serial.print(s); Serial.print(" "); Serial.println((uint)offset,HEX);
      if (!offset || !strFlt(s,offset,&s)) {
        return false;
      }
    }
    else if (strMatch(s,"duration",&s)) {
      Serial.print("parsed duration"); Serial.println(s);
      if (!duration || !strFlt(s,duration,&s)) {
        return false;
      }
    }
    else if (!offset || !strFlt(s,offset,&s)) {
      Serial.print(" found !offset or !strFlt "); Serial.println(s);
      return false;
    }
  }
  return true;
...
bool strStartNoStart(char *pc, bool *start, char **next=0)
{
  while (pc && (*pc)) {
    if (strMatch(pc,"nostart",next)) {
      *start = false;
    }
    if (strMatch(pc,"start",next)) {
      *start = true;
    }
    else {
      return false;
...
void help(const char *key) {
  ...
  if (all || strMatch(key,"lccd",&s) || strMatch(key,"read",&s) || strMatch(key,"trigger",&s)) {
    ...
    Serial.println("#");
    Serial.println("#  a.  Read the TCD1304");
    Serial.println("#");
    ...
    Serial.println("#");
    Serial.println("#     wait read [timeout [timestep]] [noblocking]");
    Serial.println("#");
    Serial.println("#  b.  Measure electronic offset, report ADU and voltage");
    Serial.println("#");
    Serial.println("#     read offset                              - measure the offset, report ADU and volts");
    Serial.println("#");
    Serial.println("#  c.  Trigger the above TCD1304 read from a digital i/o pin input");
    Serial.println("#");
    ...
    Serial.println("#");
    Serial.println("#     wait trigger [timeout [timestep]] [noblocking]");
    Serial.println("#");
    Serial.println("#     gate frame <n>");
    Serial.println("#");
    Serial.println("#     configure trigger pin <n> | keywords ...    -   configure trigger pin and edge");
    Serial.println("#       keywords:   pullup | pulldown | nopull | rising | falling | change");
    Serial.println("#");
  }
  if (all || strMatch(key,"lccd",&s) || strMatch(key,"timing",&s)) {
    Serial.println("#  d.  Configure timings for the read functions:");
    Serial.println("#");
    Serial.println("#      set clock <clk_period [<idle/exposure sub-clock>]  - sets timing parameters based on clk");
    Serial.println("#           recommended clk periods: 0.4E-6, 0.6E-6 .... 1.2E-6   (even multiples of 100nsec)");
    Serial.println("#           recommended sub clocks:  10E-6, 20E-6, 25E-6, 50E-6   (convenient for exposure time)");
    Serial.println("#");
    Serial.println("#      set delay <secs>                        - delay for the pulse mode expsure timer, use with trigger");
    Serial.println("#");
    Serial.println("#      configure timing [clk val] [name opition val] .....  - configure individual timing parameters");
    Serial.println("#");
    Serial.println("#          args:      clk val | period val | <name> <option> <val>");
    Serial.println("#          names:     sh | icg | cnvst");
    Serial.println("#          options:   duration | offset");
    Serial.println("#          vals:      time in seconds");
    Serial.println("#");
    Serial.println("#       example:");
    Serial.println("#         configure timing clk 1E-6 sh duration 1.E-6 offset 0.5E-6 icg offset 0.4E-6 duration 2.1E-6");
    Serial.println("#");
  }
  if (all || strMatch(key,"lccd",&s) || strMatch(key,"pulse",&s)) {
    ...
    Serial.println("#");
    Serial.println("#          defaults to setup from set clock and configure timing, see above.");
    Serial.println("#");
    ...
    Serial.println("#      wait pulse  - stops ongoing pulse");
    Serial.println("#");
    ...
    Serial.println("#");
    Serial.print("#      configure clearing pulses [n [period(secs)]] - power-up default is "); Serial.println(SH_CLEARING_DEFAULT);
    Serial.println("#         (SH is pulsed before the start of the next exposure to clear residual charge)");
    Serial.println("#");
  }
  if (all || strMatch(key,"lccd",&s) || strMatch(key,"pulse loop",&s)) {
    Serial.println("#  3) Pulse loop setup (time syntax: 1E-6, 0.000001 or 1u)");
    Serial.println("#");
    Serial.println("#    setup pulse loop [<clk_period> <sh_width> <sh_offset> <icg_width> <icg>_offset> [nframes [nframesets [period]]] (in secs)");
    Serial.println("#");
    Serial.println("#          defaults to configured timings - see above");
    Serial.println("#");
    Serial.println("#");
    Serial.println("#      start pulse loop  - launches the pulse loop with readout");
    Serial.println("#      arm pulse loop    - mnually prepares to relaunch, is left armed by the above");
    Serial.println("#      stop pulse loop   - stops ongoing pulse");
    Serial.println("#      wait pulse loop   - stops ongoing pulse");
    Serial.println("#");
  }
  if (all || strMatch(key,"lccd",&s)|| strMatch(key,"timer",&s)) {
    Serial.println("#  4) Timer setup, clocks pulses, implemented in the periodic interrupt timer (PIT)");
    Serial.println("#");
  ...
  if (all || strMatch(key,"lccd",&s)|| strMatch(key,"trigger",&s)) {
    Serial.println("#  5) External trigger setup - triggers pulse, timer or pulse loop (frameset ensemble) (see section 1 above)");
    Serial.println("#");
  ...
  if (all || strMatch(key,"help",&s)) {
    ...
    Serial.println("#         lccd | pulse | loop | timer | read | trigger | timing | help]");
    Serial.println("#");
  }
}
  ...
  if (nlen > 0) {
    ...
    */
    if (strMatch(rcvbuffer, "version", &pc)) {
      ...
      sprintf(sndbuffer, "PIXELS %u START %u DARK %u STOP %u BITS %u VFS %f",
              NPIXELS, DARKSTART, NDARK, DATASTOP, NBITS, VFS);
      Serial.print(sndbuffer);
    ...
    else if (strMatch(rcvbuffer, "wait", &pc)) {
      float timeout = 0.;
      float timestep = 0.01;
      bool verbose = false;
      bool interruptible = true;

      if (strMatch(pc,"read",&pc)) {
        strWaitParameters(pc, &timeout, &timestep, &interruptible, &verbose);
        tcd1304device.wait_read(timeout,timestep, interruptible, verbose);
      }
      
      else if (strMatch(pc,"triggered read",&pc)) {
        strWaitParameters(pc, &timeout, &timestep, &interruptible, &verbose);
        tcd1304device.wait_triggered_read(timeout,timestep, interruptible, verbose);
      }
      
      else if (strMatch(pc,"trigger",&pc)) {
        strWaitParameters(pc, &timeout, &timestep, &interruptible, &verbose);
        tcd1304device.wait_trigger(timeout,timestep, interruptible, verbose);
      }
      
      else if (strMatch(pc,"pit",&pc) || strMatch(pc,"timer",&pc)) {
        strWaitParameters(pc, &timeout, &timestep, &interruptible, &verbose);
        tcd1304device.pit_wait(timeout,timestep, interruptible, verbose);
      }

      else if (strMatch(pc,"pulse",&pc)) {
        strWaitParameters(pc, &timeout, &timestep, &interruptible, &verbose);
        tcd1304device.pit_wait(timeout,timestep, interruptible, verbose);
      }

      else if (strMatch(pc,"flexpwm",&pc)) {
        strWaitParameters(pc, &timeout, &timestep, &interruptible, &verbose);
        tcd1304device.flexpwm_wait(timeout,timestep, interruptible, verbose);
      }
    ...
    else if (strMatch(rcvbuffer, "configure timing", &pc)||strMatch(rcvbuffer, "configure read", &pc)) {

      while ( pc && (*pc) && isspace(*pc)) pc++;

      if (!strTimings(pc)) {
        Serial.println("Error: failed to parse configure read parameters");
      }

      // show the settings
      tcd1304device.print_pulse_parameters();
    }

    else if (strMatch(rcvbuffer, "set clk", &pc)||strMatch(rcvbuffer, "set clock", &pc)) {
      float clk_period_secs = tcd1304device.pulse_clk_secs;
      float pulse_period_secs = tcd1304device.pulse_loop_period_secs;

      if (strFlt(pc,&clk_period_secs,&pc)) {
        strFlt(pc,&pulse_period_secs,&pc);
        tcd1304device.set_pulse_parameters(clk_period_secs,pulse_period_secs);
      }
    ...
    else if (strMatch(rcvbuffer, "set delay", &pc)) {

      strFlt(pc,&tcd1304device.pit_delay_secs,&pc);
        
      // show the settings
      tcd1304device.print_pulse_parameters();
    }

    else if (strMatch(rcvbuffer, "read offset", &pc)) {
      tcd1304device.read_adc_offset();
    }
    ...
    // ---------------------------------------------------------------------------------
      ...
      // pulse loop, framesets frames, exposure and frame interval
      ...
      // timed single pulses, framesets, nframes, exposure only
      ...
      // default to 1 set of timed single pulse frames
      else if (strUint(pc,&nframes, &pc1)
               && strFlt(pc1,&exposure, &pc1)
               && strFlt(pc1,&frameinterval, &pc)) {
    ...
    /* ========================================================
     */
    else if (strMatch(rcvbuffer, "setup pit", &pc)||strMatch(rcvbuffer, "setup timer", &pc)){
	  ...
	  strFlt(pc,&exposure_offset,&pc);
        ...
        tcd1304device.pit_delay_secs = exposure_offset;
        tcd1304device.pit_interval_secs = exposure;
        tcd1304device.pit_counts = nframes+1;
        tcd1304device.setup_pit(exposure, exposure_offset, nframes+1);
      }
      else {
        Serial.println("Error: not able to parse setup pit <exposure> [N [offset]] (secs)");
      }
    }
    
    else if (strMatch(rcvbuffer, "stop pit", &pc)||strMatch(rcvbuffer, "stop timer", &pc)){
      tcd1304device.stop_pit();
    }
    
    else if (strMatch(rcvbuffer, "begin pit", &pc)||strMatch(rcvbuffer, "begin timer", &pc)){
      tcd1304device.begin_pit();
    }
    
    else if (strMatch(rcvbuffer, "start pit", &pc)||strMatch(rcvbuffer, "begin timer", &pc)){
      tcd1304device.pit_start();
    }
     ...
     */
      ...
      if (tcd1304device.mode==NOTCONFIGURED) {
        Serial.println("Error: not configured, use setup pulse, pulse loop or timer first");
      }
    ...
    else if (strMatch(rcvbuffer, "print setup", &pc)) {
      tcd1304device.print_setup();
    }

    /* ========================================================
     */
    else if (strMatch(rcvbuffer, "setup tcd1304 pulse loop", &pc) ||
             strMatch(rcvbuffer, "setup pulse loop", &pc) ||
             strMatch(rcvbuffer, "setup pulse_loop", &pc)) {

      float clk_secs = tcd1304device.pulse_clk_secs;
      float sh_secs = tcd1304device.pulse_sh_secs;
      float sh_offset_secs = tcd1304device.pulse_sh_offset_secs;
      float icg_secs = tcd1304device.pulse_icg_secs;
      float icg_offset_secs = tcd1304device.pulse_icg_offset_secs;            
      float period_secs = tcd1304device.pulse_loop_period_secs;
      
      float exposure_secs = 0.5E-3;
      float frame_interval_secs = 10.E-3;
      ...
      if ((!(pc=nextWord(pc)) || (strFlt(pc,&exposure_secs, &pc) &&
          ...
          (!(pc=nextWord(pc)) || (strFlt(pc,&clk_secs, &pc) &&
                                  ...
                                  strFlt(pc,&period_secs, &pc))
          )
         )
        {
        
          serialPrintlnf("clk %.6gs sh %.6gs +%.6gs icg %.6gs +%.6gs exp %.6gs inter %.6gs %d",
                         clk_secs, sh_secs, sh_offset_secs, icg_secs, icg_offset_secs,
                         exposure_secs, frame_interval_secs, nframes);
          
          if (tcd1304device.setup_pulse_loop(clk_secs, sh_secs, sh_offset_secs,
                                             icg_secs, icg_offset_secs, period_secs,
                                             exposure_secs, frame_interval_secs, nframes,
        ...
        Serial.println("Error: pulse loop parameters not recognized, see help frameset.");
      }
    }
      
    else if (strMatch(rcvbuffer, "start pulse loop", &pc)||strMatch(rcvbuffer, "start pulse_loop", &pc)) {
      tcd1304device.pulse_loop_start( );
    }

    else if (strMatch(rcvbuffer, "arm pulse loop", &pc)||strMatch(rcvbuffer, "arm pulse_loop", &pc)) {
      tcd1304device.pulse_loop_arm( );
    }

    else if (strMatch(rcvbuffer, "stop pulse loop", &pc)||strMatch(rcvbuffer, "stop pulse_loop", &pc)) {
      tcd1304device.pulse_loop_stop( );
    }
     ...
     */
      ...
      if (strUint(pc,&tcd1304device.sh_clearing_counts,&pc)) {
        strFlt(pc,&tcd1304device.sh_clearing_period_secs,&pc);
      }

      Serial.printf("clearing pulses %d %4g\n",
                    tcd1304device.sh_clearing_counts,
                    tcd1304device.sh_clearing_period_secs);
    }
    ...
    */
     ...
     */
      ...
      while(pc && (*pc)) {
        ...
        if (strMatch(pc, "high", &pc)||strMatch(pc, "hi", &pc)) {
          digitalWriteFast(u8tmp, HIGH);
          serialPrintlnf("Pin %d set HIGH", u8tmp);
        } 
    
        else if (strMatch(pc, "low", &pc)||strMatch(pc, "lo", &pc)) {
          digitalWriteFast(u8tmp, LOW);
          serialPrintlnf("Pin %d set LOW", u8tmp);
        }

        else if (strMatch(pc, "output", &pc)||strMatch(pc, "out", &pc)) {
          pinMode(u8tmp, OUTPUT);
          serialPrintlnf("Pin %d set OUTPUT", u8tmp);
        }
      
        else if (strMatch(pc, "input", &pc)||strMatch(pc, "in", &pc)) {
          pinMode(u8tmp, INPUT);
          serialPrintlnf("Pin %d set INPUT", u8tmp);
        }      

        else if (strMatch(pc, "nopullup", &pc)||strMatch(pc, "nopull", &pc)) {
          pinMode(u8tmp, INPUT);
          serialPrintlnf("Pin %d set INPUT", u8tmp);
        }
          ...
          serialPrintlnf("Pin %d set PULLUP", u8tmp);
        }
      
        else if (strMatch(pc, "pulldown", &pc)) {
          pinMode(u8tmp, INPUT_PULLDOWN);
          serialPrintlnf("Pin %d set PULLDOWN", u8tmp);
        }
    ...
    else if (strMatch(rcvbuffer, "pulse", &pc) && strPin(pc,&u8tmp,&pc)) {
    ...
    else if (strMatch(rcvbuffer, "toggle", &pc) && strPin(pc,&u8tmp,&pc)) {
