...
public:
  ...
  inline static float sh_clearing_period_secs = 0.;
  inline static unsigned int sh_clearing_period_counts = 0.;
  ...
  */
  ...
  static void pulse_sh_isr()
  {
    ...
    // first pulse
	 ...
	 after completing the present period. */
      flexpwm->MCTRL |= FLEXPWM_MCTRL_CLDOK(SH_MASK);
      flexpwm->SM[SH_SUBMODULE].VAL1 = sh_clearing_period_counts;
      flexpwm->MCTRL |= FLEXPWM_MCTRL_LDOK(SH_MASK);
                   ...
                   uint16_t *buffer, unsigned int nbuffer, void (*callbackf)())
  {
    ...
    if (sh_clearing_counts) {
      ...
      unsigned int sh_period_counts_min;

      // the requested sh clearing period
      sh_clearing_period_counts = ceil(sh_clearing_period_secs*F_BUS_ACTUAL/sh.divider);

      // the initial period -  has to be at least after the icg and one clock
      sh_period_counts_min = icg.offA_counts + clk.period_counts;
      sh.period_counts = sh_clearing_period_counts < sh_period_counts_min ? sh_period_counts_min : sh_clearing_period_counts;

      // the subsequent intervals - set the minimum at twice the SH pulse length (square wave)
      sh_period_counts_min = 2*(sh.offA_counts - sh.onA_counts) + 1;
      sh_clearing_period_counts = sh_clearing_period_counts < sh_period_counts_min ? sh_period_counts_min : sh_clearing_period_counts;
    ...
    Serial.println("");
    Serial.printf("flexpwm: clearing_pulses %d\n", sh_clearing_counts);
    //Serial.printf("flexpwm: clearing_pulse_period_counts %d\n", sh_clearing_period_counts);
    Serial.printf("flexpwm: clearing_pulse_period_secs %.6g\n", (float)(sh_clearing_period_counts*sh.divider)/F_BUS_ACTUAL);
...
#undef DEBUG
    ...
    unsigned int ncp = period_secs > 0 ? (frame_secs-exposure_secs)/period_secs : 0;
    Serial.println("");
    Serial.printf("flexpwm: clearing_pulses %d\n", ncp);
    Serial.printf("flexpwm: clearing_pulse_period_secs %.6g\n", period_secs);
