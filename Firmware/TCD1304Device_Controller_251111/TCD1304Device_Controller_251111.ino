...
void send_frame_header( )
{
  ...
  if (tcd1304device.trigger_mode) {
    ...
    Serial.println(tcd1304device.trigger_elapsed_secs( ),8);
    Serial.print("TRIGGER DIFFERENCE ");
    Serial.println(tcd1304device.trigger_difference_secs( ),8);
    Serial.print("TRIGGER COUNTER ");
    Serial.println(tcd1304device.trigger_counter);
...
void loop( ) {
  ...
  if (nlen > 0) {
    ...
    */
    ...
    /* ========================================================
     */
    else if (strMatch(rcvbuffer, "clear accumulator", &pc)) {
      clearAccumulator( );
      Serial.println("tcd1304: cleared the accumulator");
    }

    else if (strMatch(rcvbuffer, "start accumulator", &pc)) {
      startAccumulator( );
      Serial.println("tcd1304: start the accumulator");
    }
    
    else if (strMatch(rcvbuffer, "stop accumulator", &pc)) {
      stopAccumulator( );
    ...
    */
    else if (strMatch(rcvbuffer, "test adc", &pc)) {
      unsigned int usecs;
